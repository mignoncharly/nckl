...
if [[ -n "$staged_files" ]]; then
  ...
  diff_targets=$(echo "$staged_files" | grep -v -E '(^|/)(\.env\.example|scripts/check-repo-safety\.sh|backend/config/settings/base\.py|frontend/package-lock\.json|deploy/\.secrets\.example\.json|deploy/redis-nckl\.conf\.example|deploy/root_setup\.sh|docs/audit/.*|docs/implementation/.*)$' || true)
  staged_secret_hits=""
